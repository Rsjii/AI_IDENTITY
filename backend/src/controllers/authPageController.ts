...
import { userQueries, identityQueries } from '../config/database';
  ...
  try {
    if (req.user && req.user.email) {
      ...
      if (fullUser && fullUser.profileCompleted) {
        // Fully onboarded → check identity and redirect
        const redirectUrl = await getPostLoginRedirect(fullUser.id);
