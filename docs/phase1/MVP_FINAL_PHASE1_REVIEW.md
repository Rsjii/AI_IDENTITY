...
2) **AI ready email**
   - ✅ Fixed: email now sent once when training completes.

3) **Public session history (30 days)**
   - ✅ Fixed: session ID persisted + history API added.

4) **Language fallback to English**
   - ✅ Fixed: default language now falls back to English.

5) **Minimum content requirement**
   - ✅ Fixed: onboarding blocks until 3+ items uploaded.
...
1) **Pay‑per‑chat verification**
   - ✅ Fixed: confirm now validates Stripe metadata + idempotent.
...
3) **Chat history API**
   - ✅ Done: history endpoint + client load on mount.
